...
interface crvIBdeposit:
    def add_liquidity(uamounts: uint256[3], min_mint_amount: uint256): nonpayable
...
@internal
def recycle_exact_amounts(sender: address, _dai: uint256, _usdc: uint256, _usdt: uint256, _crvIB: uint256):
    if _dai > 0:
    ...
    if _crvIB > 0:
        ERC20(crvIB).transferFrom(sender, self, _ycrv)

    deposit_amounts: uint256[3] = [_dai, _usdc, _usdt]
    if _dai + _usdc + _usdt > 0:
        crvIBdeposit(ibdeposit).add_liquidity(deposit_amounts, 0)
    ...
    log Recycled(sender, _dai, _usdc, _usdt, _crvIB, _yvcrvIB)
...
def recycle():
    ...
    _crvIB: uint256 = min(ERC20(crvIB).balanceOf(msg.sender), ERC20(crvIB).allowance(msg.sender, self))

    self.recycle_exact_amounts(msg.sender, _dai, _usdc, _usdt, _crvIB)


@external
def recycle_exact(_dai: uint256, _usdc: uint256, _usdt: uint256, _crvIB: uint256):
    self.recycle_exact_amounts(msg.sender, _dai, _usdc, _usdt, _crvIB)
